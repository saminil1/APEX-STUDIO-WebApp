...
export default function DetailPageContent() {
  const [selected, setSelected] = useState<DetailCategory>("전체");

  const filtered = useMemo(() => {
    return DETAIL_WORKS.filter(
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
          <AnimatePresence mode="popLayout">
            {filtered.map((item) => (
              ...
              >
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden mb-3 bg-[#111]">
                  <Image
                    src={item.img}
                    alt={item.name}
                    fill
